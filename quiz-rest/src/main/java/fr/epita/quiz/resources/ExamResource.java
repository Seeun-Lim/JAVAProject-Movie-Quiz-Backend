...
//		User user = new User();
//		
//		user.setLoginName(answerDTO.getLoginName());
//		System.out.println("got username with:" + answerDTO.getLoginName());
//		
...
//	}
	...
	@POST
	@Path("/answer/{loginName}")
	@Consumes(value = MediaType.APPLICATION_JSON)
	public Response addUserToQuestion(@RequestBody AnswerDTO answerDTO) {
		...
		try {
			return Response.created(new URI("/rest/exam/answer/" + user.getLoginName())).build();
		} catch (URISyntaxException e) {
			// TODO Handle things properly
			e.printStackTrace();
		}
	...
	public Response getAnswer(@PathParam("id") long answerId) {
		...
		Answer answer = new Answer();
		answer.setContent("This is a sampleAnswer with" + answerId);
	...
	public Response getAnswers() {
		...
		answer.setContent("This is a sampleAnswer when you run on a server");
